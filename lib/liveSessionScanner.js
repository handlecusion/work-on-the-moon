...
async function scanLive() {
  ...
  _inflight = (async () => {
    try {
      ...
      const [enriched, cmuxProbe, tmuxProbe, codexResults] = await Promise.all([
      ...
      // Concatenate claude (already tagged agent='claude' in enrich) and codex
      // (already tagged agent='codex' in scanCodex) before decoration.
      const result = enriched.filter(Boolean).concat(codexResults);
      ...
      // (env-var heuristic). For codex entries, source starts at 'terminal' and
      // is promoted to 'cmux' here purely by cwd→workspace match.
      for (const entry of result) {
        if (entry.source === 'cmux') {
          // already correct — cmux wins (claude env-var heuristic)
        } else if (entry.agent === 'codex' && entry.cmuxAvailable) {
          entry.source = 'cmux';
