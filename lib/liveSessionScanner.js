...
function findFallbackSessionId(cwd, startedAtMs) {
  ...
  // Cutoff = PID start time (5s grace for clock skew/process startup).
  // Any jsonl modified after that belongs to this process, regardless
  // of current idle. If startedAt is unknown, keep the legacy 60s window.
  const cutoff = startedAtMs
    ? startedAtMs - 5000
