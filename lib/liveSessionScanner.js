...
async function enumeratePids() {
  const r = await execP('pgrep', ['-fla', 'local/bin/claude|^claude$|^claude\\s']);
  if (!r.stdout) return [];
  const out = [];
...
async function enrich(proc) {
  try {
    const [cwdResolved, startedAt] = await Promise.all([
      probeCwd(proc.pid),
...
async function scanLive() {
  ...
  _inflight = (async () => {
    try {
      ...
      // Run per-PID enrichment and cmux probe in parallel to keep total latency
      // low. The cmux probe has its own 1.5s budget; per-PID enrichment typically
      // finishes in ~150ms, so both should complete before the budget elapses.
      const [enriched, cmuxProbe] = await Promise.all([
