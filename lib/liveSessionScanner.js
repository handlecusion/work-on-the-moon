...
const codexScanner = require('./codexSessionScanner');
const hermesScanner = require('./hermesSessionScanner');
...
function invalidateCache() {
  ...
  try { codexScanner.invalidateCache(); } catch (_) {}
  try { hermesScanner.invalidateCache(); } catch (_) {}
}
...
function findFallbackSessionId(cwd, startedAtMs, excludeIds) {
  if (!cwd) return null;
  ...
  // Collect all candidate jsonl files with their stat metadata once.
  const candidates = [];
  for (const e of entries) {
    ...
    candidates.push({
      ...
      birthMs: st.birthtime ? st.birthtime.getTime() : st.mtime.getTime()
    });
  }
  if (candidates.length === 0) return null;

  // Primary: when we know when the PID started, match the jsonl whose
  // birthtime is closest to startedAt. This uniquely identifies which file
  // a given process owns when multiple `claude` instances share a cwd
  // (otherwise the most-recent-mtime fallback collapses them onto the same
  // jsonl, producing identical previews on the home page).
  if (startedAtMs) {
    let best = null;
    for (const c of candidates) {
      // Created at or after PID start (with grace for clock skew). Files
      // born well before this PID started belong to a different process.
      if (c.birthMs < startedAtMs - 5000) continue;
  ...
  // Fallback: most-recently-modified within window. Used when birthtime
  // matching failed (e.g. `claude --continue` re-attaching to an existing
  // jsonl, where birthtime is older than startedAt) or startedAt unknown.
  const cutoff = startedAtMs ? startedAtMs - 5000 : Date.now() - FALLBACK_LOOKBACK_MS;
  let best = null;
  for (const c of candidates) {
    if (c.mtimeMs < cutoff) continue;
    if (!best || c.mtimeMs > best.mtimeMs) best = { id: c.id, mtimeMs: c.mtimeMs };
  }
...
// Load jsonl head/tail summary for a given (cwd, sessionId) pair. Returns
// canonical-empty when sessionId is missing or the file does not exist.
function describeSession(cwd, sessionId) {
  let jsonlPath = null;
  ...
  if (sessionId) {
    const candidate = jsonlPathFor(cwd, sessionId);
  ...
  return { jsonlPath, head, tail };
}

function deriveBusyAndIdle(sessionId, jsonlPath, tail) {
  let idleSeconds;
...
async function enrich(proc) {
  try {
    ...
    const cmdSessionId = extractSessionId(proc.command);
    let sessionId = cmdSessionId;
    if (!sessionId) {
      const startedAtMs = startedAt ? Date.parse(startedAt) : null;
      sessionId = findFallbackSessionId(cwdResolved, startedAtMs);
    }

    const source = detectSource(proc.command);
    const argsSummary = summarizeArgs(proc.command);

    const { jsonlPath, head, tail } = describeSession(cwdResolved, sessionId);
    const { busy, idleSeconds } = deriveBusyAndIdle(sessionId, jsonlPath, tail);
...
/**
 * Resolve sessionId collisions among claude entries that share a cwd. Without
 * this, two `--continue` processes in the same project both fall through to
 * `findFallbackSessionId`'s most-recent-mtime branch and end up pointing at
 * the same jsonl — producing two rows on the home page that both navigate to
 * `/chat-live/<same-sid>`.
 *
 * Strategy: PIDs that pulled their sessionId from the command line (via
 * --session-id / --continue <uuid> / --resume <uuid>) are authoritative and
 * keep their claim. Fallback-resolved PIDs in the same cwd are then sorted by
 * startedAt ascending so the oldest claims first; later starters whose sid
 * collides with an already-claimed one re-run findFallbackSessionId with the
 * claimed sids excluded. If no alternative jsonl exists, the duplicate is
 * downgraded to a sid-less entry (router will use the cwd-keyed route).
 */
function deduplicateClaudeSessions(entries) {
  const byCwd = new Map();
  for (const e of entries) {
    if (!e || e.agent !== 'claude') continue;
    const key = e.cwdResolved || e.cwd;
    if (!key) continue;
  ...
  for (const [cwd, list] of byCwd) {
    ...
    for (const e of list) {
      if (e._fromCmd && e.sessionId) claimed.add(e.sessionId);
    }
    const fallbackEntries = list.filter((e) => !e._fromCmd);
    ...
    for (const e of fallbackEntries) {
      if (e.sessionId && claimed.has(e.sessionId)) {
        const startedAtMs = e.startedAt ? Date.parse(e.startedAt) : null;
        const newSid = findFallbackSessionId(cwd, startedAtMs, claimed);
        if (newSid && newSid !== e.sessionId) {
          e.sessionId = newSid;
          const { jsonlPath, head, tail } = describeSession(cwd, newSid);
          const { busy, idleSeconds } = deriveBusyAndIdle(newSid, jsonlPath, tail);
          e.jsonlPath = jsonlPath;
          e.gitBranch = head.gitBranch || e.gitBranch;
          e.lastEntryRole = tail.lastEntryRole;
          e.lastUserText = tail.lastUserText;
          e.lastAssistantText = tail.lastAssistantText;
          e.lastActivityTs = tail.lastActivityTs || tail.mtime;
          e.busy = busy;
          e.idleSeconds = idleSeconds;
        } else {
          // No spare jsonl. Strip sid so the row routes via /chat-live-cwd/.
          e.sessionId = null;
          e.jsonlPath = null;
          e.lastEntryRole = null;
          e.lastUserText = null;
          e.lastAssistantText = null;
          e.busy = true;
          e.idleSeconds = 0;
        }
      }
      if (e.sessionId) claimed.add(e.sessionId);
    }
  }
  for (const e of entries) {
    if (e && e.agent === 'claude') delete e._fromCmd;
  }
}
...
async function scanLive() {
  ...
  _inflight = (async () => {
    try {
      ...
      // Run per-PID enrichment, cmux probe, tmux probe, and codex scan in
      // parallel to keep total latency low. Each probe has its own budget.
      // codex failures must not break claude results — soft-fail to [].
      const codexPromise = codexScanner.scanCodex().catch((err) => {
        console.log('[scanner] codex scan failed: ' + (err && err.message || err));
        return [];
      });
      const hermesPromise = hermesScanner.scanHermes().catch((err) => {
        console.log('[scanner] hermes scan failed: ' + (err && err.message || err));
        return [];
      });
      const [enriched, cmuxProbe, tmuxProbe, codexResults, hermesResults] = await Promise.all([
      ...
      // Concatenate claude (tagged agent='claude' in enrich), codex
      // (tagged agent='codex' in scanCodex), and hermes (tagged agent='hermes'
      // in scanHermes) before decoration.
      const claudeEntries = enriched.filter(Boolean);
      deduplicateClaudeSessions(claudeEntries);
      const result = claudeEntries.concat(codexResults).concat(hermesResults);
      ...
      // A cmux cwd → workspace match is intentionally fuzzy: it can only say
      // "there is a cmux terminal somewhere in this cwd". When several live
      // agents share the same cwd (common in Placet), that fuzzy match can be
      // a different surface than the PID/jsonl row the user opened. A tmux
      // match, by contrast, is TTY/PID-bound and therefore identifies the exact
      // pane for this process. If both are present and the process was not
      // explicitly launched by cmux, suppress the fuzzy cmux transport so input
      // cannot be routed into a sibling terminal session.
      for (const entry of result) {
        if (entry.tmuxAvailable && entry.source !== 'cmux') {
          entry.cmuxAvailable = false;
          entry.cmuxWorkspaceId = null;
          entry.cmuxSurfaceId = null;
        }
      ...
      // Refine source using routing priority: exact cmux launch > exact tmux
      // pane > cwd-inferred cmux > terminal.
      // For claude entries, source may already be 'cmux' from detectSource(cmd)
      // (env-var heuristic). For codex/hermes entries, source starts at
      // 'terminal' and is promoted to 'cmux' here purely by cwd→workspace match.
      for (const entry of result) {
        if (entry.source === 'cmux') {
          // already correct — cmux wins (claude env-var heuristic)
        } else if (entry.tmuxAvailable) {
          entry.source = 'tmux';
        } else if ((entry.agent === 'codex' || entry.agent === 'hermes') && entry.cmuxAvailable) {
          entry.source = 'cmux';
        } else {
      ...
      // Sort: lastActivityTs desc, then agent asc (claude before codex when
      // ties), then pid asc. Null lastActivityTs sinks to the bottom.
      result.sort((a, b) => {
