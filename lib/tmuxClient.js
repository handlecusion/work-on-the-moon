...
async function sendText(socketPath, paneId, text) {
  ...
  // Peel trailing newline. Bracketed-paste-aware TUIs (claude TUI included)
  // treat \n inside the paste sequence as Shift+Enter (newline), not submit.
  // The real Enter must arrive outside the paste — sent as a separate key.
  let body = text;
  let pressEnterAfter = false;
  if (body.endsWith('\n')) {
    body = body.slice(0, -1);
  ...
  if (pressEnterAfter) {
    try {
      await execP('tmux', ['-S', socketPath, 'send-keys', '-t', paneId, 'Enter'], { timeout: 2000 });
    } catch (err) {
      throw new TmuxError('send_keys_failed', 'tmux send-keys Enter failed: ' + (err.message || String(err)), err);
    }
  }
}

/**
 * capturePane(socketPath, paneId, opts) — async.
 * Returns a plain-text snapshot of the pane (no escape sequences). `opts.lines`
 * controls how many trailing lines to include (default 50, max 500).
 */
async function capturePane(socketPath, paneId, opts) {
  if (!PANE_ID_RE.test(paneId)) {
    throw new TmuxError('invalid_pane_id', 'paneId must match /^%\\d+$/');
  }
  validateSocketPath(socketPath);
  const requested = (opts && Number.isFinite(opts.lines)) ? Math.floor(opts.lines) : 50;
  const lines = Math.min(Math.max(requested, 1), 500);
  try {
    // -p: print, -J: join wrapped lines, -t: target, -S -N: start N lines back.
    const { stdout } = await execP(
      'tmux',
      ['-S', socketPath, 'capture-pane', '-p', '-J', '-t', paneId, '-S', '-' + lines],
      { timeout: 1500 }
    );
    return stdout;
  } catch (err) {
    throw new TmuxError('capture_pane_failed', 'tmux capture-pane failed: ' + (err.message || String(err)), err);
  }
}
