...
const fs = require('fs');
const defaultNormalizer = require('./jsonlNormalizer');
...
function _processGrowth(entry, jsonlPath, newSize) {
  ...
  const normalizeEntry = entry.normalizer.normalizeEntry;
...
function _checkFile(jsonlPath) {
  ...
  if (newSize < sizeBefore) {
    ...
    entry.byteOffset = 0;
    entry.partialLine = '';
    const events = entry.normalizer.loadEntireTranscript(jsonlPath);
 ...
 *
 * `opts.normalizer` (optional) — module exporting `{ normalizeEntry,
 * loadEntireTranscript }`. Defaults to the claude jsonlNormalizer for
 * back-compat. Pass the codex normalizer for codex sessions. Once an entry
 * is created for a path, the normalizer is fixed for the file's lifetime
 * (subsequent attach calls reuse the existing entry and ignore the param).
 *
 * Returns a handle with `.detach()` that removes this listener.
 */
async function attach(jsonlPath, listener, opts) {
  ...
  const normalizer = (opts && opts.normalizer) || defaultNormalizer;
  ...
  const replay = entry.normalizer.loadEntireTranscript(jsonlPath);
