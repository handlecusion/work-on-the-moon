...
async function resolveActiveSessionCwds() {
  const sql =
    ...
    '       m.timestamp AS timestamp, m.id AS message_id,' +
    ...
    'ORDER BY c.active_id, c.depth ASC, m.timestamp DESC, m.id DESC';
  const rows = await runQuery(sql);
  return resolveCwdMapFromOnboardRows(rows);
}
...
function compareOnboardRows(a, b) {
  const activeA = String(a && a.active_id || '');
  const activeB = String(b && b.active_id || '');
  if (activeA !== activeB) return activeA < activeB ? -1 : 1;

  const depthA = rowNumber(a && a.depth, Number.MAX_SAFE_INTEGER);
  const depthB = rowNumber(b && b.depth, Number.MAX_SAFE_INTEGER);
  if (depthA !== depthB) return depthA - depthB;

  const tsA = rowNumber(a && a.timestamp, -Infinity);
  const tsB = rowNumber(b && b.timestamp, -Infinity);
  if (tsA !== tsB) return tsB - tsA;

  const idA = rowNumber(a && a.message_id, -Infinity);
  const idB = rowNumber(b && b.message_id, -Infinity);
  if (idA !== idB) return idB - idA;

  return 0;
}

function resolveCwdMapFromOnboardRows(rows) {
  const map = {};
  const orderedRows = Array.isArray(rows) ? rows.slice().sort(compareOnboardRows) : [];
  for (const row of orderedRows) {
    const id = row.active_id;
    if (!id || map[id]) continue; // sorted rows choose closest, then newest
