...
const MAX_HEAD_BYTES   = 1024 * 1024; // upper bound on first-line read to defend against
                                       // pathological/unterminated files. The codex session_meta
                                       // line was ~22KB on CLI 0.130.0 and is expected to grow as
                                       // base_instructions evolves; we read in chunks until the
                                       // first '\n' instead of capping to a fixed slice.
...
/**
 * Read the first newline-terminated line of a file as UTF-8, reading in
 * chunks until either '\n' is found or `maxBytes` total bytes have been
 * read. Returns '' on failure or empty file. The trailing '\n' is stripped.
 *
 * Used for the codex session_meta head, which inlines `base_instructions.text`
 * and therefore varies in size across CLI versions — a fixed-size slice
 * truncated mid-JSON whenever the blob outgrew the cap.
 */
function readFirstLine(filePath, maxBytes) {
  ...
  try {
    ...
    let read = 0;
    while (read < cap) {
      const want = Math.min(CHUNK, cap - read);
      const got = fs.readSync(fd, buf, 0, want, read);
      if (got <= 0) break;
      const nlIdx = buf.indexOf(0x0a);
      if (nlIdx !== -1 && nlIdx < got) {
        collected.push(Buffer.from(buf.subarray(0, nlIdx)));
        return Buffer.concat(collected).toString('utf8');
      }
      collected.push(Buffer.from(buf.subarray(0, got)));
      read += got;
    }
    return Buffer.concat(collected).toString('utf8');
  } catch (_) {
...
function parseCodexHead(filePath) {
  const firstLine = readFirstLine(filePath, MAX_HEAD_BYTES);
