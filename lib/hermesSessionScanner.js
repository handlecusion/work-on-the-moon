...
async function enumeratePids() {
  ...
  for (const raw of lines) {
    ...
    // Require an explicit chat-mode signal in argv. Without this filter,
    // bare `hermes` invocations (TUI shells, idle interpreters, etc.) get
    // listed as attachable rows that can never be matched to a jsonl —
    // the user just sees a row leading to an infinite-loading page.
    //
    // Accepted shapes (all imply the process is in / will enter a chat):
    //   hermes chat …
    //   hermes -q QUERY            (alias for `hermes chat -q`)
    //   hermes -z QUERY            (older alias kept for back-compat)
    //   hermes --tui               (interactive TUI chat shell)
    //   hermes -c | hermes --continue …
    //   hermes -r | hermes --resume …
    const isChatLike = /(?:^|\s)(?:chat|--tui|-q|--query|-z|-c|--continue|-r|--resume)(?:\s|$)/.test(cmd);
...
async function enrich(proc) {
  try {
    ...
    // No matchable jsonl → drop this pid from the listing. Otherwise the
    // home page renders a row whose only link is `/chat-live-cwd/<cwd>`
    // and chat-live polls forever waiting for a jsonl that won't arrive.
    if (!match) return null;

    const sessionId = match.sessionId;
    const jsonlPath = match.jsonlPath;
