 ...
 *
 * Strategy: the DB lists every CLI session with `ended_at IS NULL` sorted
 * by most recent message timestamp. We pick the freshest one whose last
 * activity falls AT OR AFTER this pid's start time — i.e., the session
 * the user has been sending messages to since this hermes process began.
 *
 * `availableSessions` is the result of hermesDb.listActiveCliSessions()
 * passed in once per scan so we don't requery for each pid.
 */
function pickSessionForPid(startedAtMs, availableSessions) {
  if (!availableSessions || !availableSessions.length) return null;
  // The DB returns rows ordered by last_ts desc, so the first match wins.
  // Allow a small grace for clock skew between process lstart and message
  // timestamp.
...
async function enrich(proc, availableSessions) {
  try {
    ...
    const session = pickSessionForPid(startedAtMs, availableSessions);
...
async function scanHermes() {
  ...
  _inflight = (async () => {
    try {
      ...
      // Greedy 1-to-1 assignment: each session can be claimed by at most
      // one pid. Iterate pids in order; pickSessionForPid picks the freshest
      // unclaimed session whose last activity post-dates that pid.
      const claimed = new Set();
      const enriched = [];
      for (const proc of procs) {
        const remaining = sessions.filter(s => !claimed.has(s.id));
        const entry = await enrich(proc, remaining);
